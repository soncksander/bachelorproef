...
# Eenvoudige vorm van Chain-of-Thought
paper_qa_prompt_str = (
    ...
    "3. HARDE DATA: Haal de specifieke getallen (zoals 30-60 g/h of 6-8%, intervallen in minuten) uit de tekst.\n"
    "Vraag: {query_str}\n"
    "Antwoord: "
)
...
# Definieer de Reranker
reranker = LLMRerank(choice_batch_size=5, top_n=3, llm=llm)

# Base engine mét Reranker en een breed zoeknet (top_k=15)
...
# Definieer de HyDE transformatie
hyde_transform = HyDEQueryTransform(include_original=True)

# Koppel alles aan elkaar in de definitieve engine
...
recipe_qa_prompt_str = (
    ...
    "STAP 3: VERPLICHTE WEIGERING: Als alle recepten in de context de foute tags hebben voor de vraag, weiger dan te antwoorden. Geef als antwoord EXACT dit: 'Geen passend recept gevonden voor deze specifieke maaltijd.' (Verzin geen smoesjes, dit moet je letterlijk outputten).\n"
...
def bereken_macros_fysioloog(vraag_sporter: str) -> str:
    """
    Gebruik deze tool ALTIJD als de gebruiker vraagt om een dagmenu, maaltijdplanning,
    of macro-berekeningen voor een specifieke training of wedstrijddag.
    Geef simpelweg de originele vraag van de gebruiker mee als input.
    Deze tool retourneert de exact berekende fysiologische richtlijnen en kloktijden.
    """
    physiologist_prompt = f"""
    Je bent een uiterst precieze sportfysioloog. Lees de onderstaande vraag van de sporter en bereken EXACT de 8 fysiologische regels.
    
    Vraag van de sporter: "{vraag_sporter}"
    
    Gebruik uitsluitend deze harde formules en definities:
    1. Totaal eiwit: 1,4 tot 2,0 * Gewicht (kg)
    2. Totaal koolhydraten: Licht (3-5 g/kg), Matig (5-7 g/kg), Zwaar (6-10 g/kg), Zeer Zwaar (8-12 g/kg).
    3. Eiwit per maaltijd: Elke maaltijd 20 tot 40 gram eiwit.
    4. Eiwitverdeling: Elke 3 tot 4 uur.
    5. Pre-workout (Let op de duur!): 
       - Bij training t/m 60 minuten: Niet vereist, geplande maaltijd volstaat. 
       - Bij training langer dan 60 minuten: 1 tot 4 uur vooraf 1 tot 4 g/kg koolhydraten.
    6. Intra-workout (Let op de duur! Vermenigvuldig dit NIET met het aantal uren, geef puur de 'per uur' richtlijn!): 
       - Bij training t/m 60 minuten: 0 gram (geen extra koolhydraten nodig).
       - Bij training langer dan 60 minuten t/m 150 minuten: 30 tot 60 gram koolhydraten per uur.
       - Bij training langer dan 150 minuten: tot 90 gram koolhydraten per uur.
    7. Snel herstel (Let op rusttijd!): 
       - Als er MINDER dan 8 uur tussen trainingen zit: de eerste 4 uur na de training 1 tot 1,2 g/kg koolhydraten per uur.
       - Als er 8 uur of meer tussen zit (of pas de volgende dag): 'Niet van toepassing'.
    8. Pre-sleep: Een snack exact 30 min voor de bedtijd met 30 tot 40 gram eiwit.

    Geef UITSLUITEND de berekende waarden terug in dit format (bereken de exacte grammen en kloktijden!):
    DEEL 1: Fysiologische Richtlijnen
    - Totaal Eiwit: [X] tot [Y] gram
    - Totaal Koolhydraten: [X] tot [Y] gram
    - Eiwit per maaltijd: 20 tot 40 gram
    - Eiwitverdeling: Elke 3 tot 4 uur
    - Pre-workout: [X] tot [Y] gram koolhydraten (tussen [Kloktijd] en [Kloktijd]) of 'Niet van toepassing'
    - Intra-workout: [Kopieer hier de letterlijke tekst '30 tot 60 gram per uur', 'tot 90 gram per uur' of 'Niet nodig'. Reken GEEN totalen uit!]
    - Snel herstel: [X] tot [Y] gram koolhydraten per uur (of 'Niet van toepassing')
    - Pre-sleep: 30 tot 40 gram eiwit om [Kloktijd]
    """
    response = llm.complete(physiologist_prompt)
    return str(response.text)

# Let op: De functienaam 'bereken_macros_fysioloog' wordt behouden omdat de agent hier letterlijk op zoekt
tool_physiologist = FunctionTool.from_defaults(fn=bereken_macros_fysioloog)
...
tool_paper = QueryEngineTool(
    ...
    metadata=ToolMetadata(
        ...
        description=(
            "Gebruik deze tool voor theorievragen over fysiologie en voeding. "
            "BELANGRIJK: De papers zijn in het Engels! VERTAAL je zoekterm ALTIJD naar wetenschappelijk Engels. "
            "SPLITS JE ZOEKOPDRACHTEN OP: Als een gebruiker vraagt naar richtlijnen tijdens inspanning, roep deze tool dan TWEE KEER achter elkaar aan met aparte, simpele queries:\n"
            "1. Zoek eerst naar de grammen: (bijv. 'recommended carbohydrate intake exact grams per hour during 2 hour exercise').\n"
            "2. Zoek daarna naar het drinken: (bijv. 'sports drink fluid concentration percentage AND exact timing intervals in minutes DURING EXERCISE ONLY, exclude pre-exercise and post-exercise').\n"
            "Combineer daarna beide antwoorden tot één compleet advies."
        ),
    ),
)
...
tool_recipe = QueryEngineTool(
    ...
    metadata=ToolMetadata(
        ...
        description=(
            ...
            "Voorbeeld: 'high-protein snack ONLY, exclude dinner and lunch tags'. "
            "De tool retourneert complete recepten."
        ),
    ),
)
...
tool_food_sql = QueryEngineTool(
    ...
    metadata=ToolMetadata(
        ...
        description=(
            ...
            "LET OP 1: Zoek NOOIT naar complete receptnamen (zoals 'High Protein Pancake'). "
            "LET OP 2: Zoek NOOIT naar abstracte termen, voedingsstoffen of supplementen (zoals 'caseïne', 'whey', 'suiker' of 'aminozuren'). "
            "LET OP 3: Zoek ALTIJD naar concrete, alledaagse basis-voedingsmiddelen. Vertaal abstracte termen zelf naar echte producten (zoek bijvoorbeeld naar 'kwark' of 'melk' in plaats van 'caseïne', en naar 'ei', 'havermout' of 'kipfilet'). "
            "Geef ALTIJD een volledige vraag mee als input, bijvoorbeeld: 'Hoeveel eiwitten zitten er in magere kwark?' of 'Wat is de voedingswaarde van quinoa?'"
        ),
    ),
)

system_prompt = (
    "Je bent een professionele sportdiëtist en behulpzame AI-assistent. "
    ...
    "- STEL NOOIT WEDERVRAGEN aan de gebruiker. Als een vraag breed of onduidelijk is, zoek dan gewoon naar de meest algemene, complete theorie in de database en geef daar antwoord op.\n"
    "- BLIJF ZOEKEN: Als je eerste zoekopdracht in de tool geen of onvoldoende informatie oplevert, wees dan creatief. Verander je zoektermen (probeer synoniemen of vertaal je zoekterm naar het Engels) en roep de tool OPNIEUW aan. Doe dit net zolang tot je een goed, feitelijk antwoord kunt vormen.\n"
    "- Gebruik géén fysioloog-tool of recepten-tool.\n"
    "- Geef een helder en feitelijk antwoord.\n\n"
    ...
    "1. Roep EERST de tool 'bereken_macros_fysioloog' aan met de vraag van de gebruiker. "
    "   Zoek NIET in de literatuur_database naar formules, vertrouw puur op de output van de fysioloog-tool.\n"
    "2. Gebruik de 'recepten_database' (en eventueel Voedingswaarde_database) om 2-3 passende, gevarieerde recepten te zoeken "
    "   bij de macro's die de fysioloog heeft berekend.\n"
    "3. Bouw je eindantwoord EXACT als volgt op:\n"
    "   \n"
    "   DEEL 1: Fysiologische Richtlijnen\n"
    "   [Kopieer hier exact de output van de fysioloog-tool]\n"
    "   \n"
...
agent = OpenAIAgent.from_tools(
    tools=[tool_paper, tool_recipe, tool_food_sql, tool_physiologist],
    llm=llm,
...
for index, (question, ground_truth) in enumerate(
    ...
    # Voeg toe aan de logging lijst (gebruikmakend van Engelse keys)
    agent_logs.append(
        {
            "Test_ID": index + 1,
...
log_filename_context = f"../output_agents/agent_evaluatie_rapport_context_{timestamp}.md"
log_filename_no_context = f"../output_agents/agent_evaluatie_rapport_{timestamp}.md"
...
# ==========================================
# 9. SLA DE DATA OP VOOR HET ANALYSE SCRIPT
# ==========================================
